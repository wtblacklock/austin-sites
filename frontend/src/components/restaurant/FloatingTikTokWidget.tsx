...
export function FloatingTikTokWidget({ videoUrl, thumbnailSrc }: FloatingTikTokWidgetProps) {
  ...
  // Convert embed URL to regular TikTok URL
  const getTikTokUrl = (embedUrl: string): string => {
    // Extract video ID from embed URL format: https://www.tiktok.com/embed/v2/{videoId}
    const match = embedUrl.match(/embed\/v2\/(\d+)/);
    if (match && match[1]) {
      return `https://www.tiktok.com/t/${match[1]}`;
    }
    return embedUrl;
  };
