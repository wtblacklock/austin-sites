...
export function GallerySlider({ images, className }: GallerySliderProps) {
  ...
  const getVisibleImages = (rowImages: GalleryImage[], reverse: boolean = false) => {
    const visibleCount = 4; // Show 4 images at a time (8 total: 4 top + 4 bottom)
          ...
          <AnimatePresence mode="wait">
            {visibleTopImages.map((imageData, idx) => (
              <motion.div
                ...
                className="relative flex-shrink-0 w-[calc(25%-0.75rem)] aspect-[4/3] overflow-hidden rounded-2xl border border-gray-200 shadow-sm hover:shadow-lg transition-shadow"
              >
                <Image
                  src={imageData.src}
                  alt={imageData.alt}
